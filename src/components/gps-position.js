AFRAME.registerSystem('gps-position', {
  ...
  _haversineDist: function (src, dest) {
    ...
    const a = (Math.sin(dlatitude / 2) * Math.sin(dlatitude / 2)) + Math.cos(THREE.MathUtils.degToRad(src.latitude)) * Math.cos(THREE.MathUtils.degToRad(dest.latitude)) * (Math.sin(dlongitude / 2) * Math.sin(dlongitude / 2));
